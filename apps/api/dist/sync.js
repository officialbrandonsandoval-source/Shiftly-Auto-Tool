 ...
 */
export async function syncProviderConnection(connectionId, dealerId, adapter, correlationId = crypto.randomUUID()) {
    const errors = [];
    ...
    try {
        ...
        for (const providerVehicle of providerVehicles) {
            try {
                const existingVehicleCheck = await import('./vehicles.js').then((m) => m.listVehicles({ dealerId, providerConnectionId: connectionId }));
                const existingVehicle = existingVehicleCheck.find((v) => v.providerId === providerVehicle.providerId);
