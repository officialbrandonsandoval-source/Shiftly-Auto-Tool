 ...
 */
export async function addPostingJob(data, delay) {
    const options = {
        attempts: 3,
        backoff: {
            type: 'exponential',
 ...
 */
export async function addRetryJob(data) {
 ...
 */
export async function addRepostJob(data, delayMs) {
    const options = {
        attempts: 3,
        backoff: {
            type: 'exponential',
 ...
 * Get job status
 */
export async function getJobStatus(jobId, queue) {
 ...
 */
export async function cancelJob(jobId, queue) {
    const job = await queue.getJob(jobId);
    if (!job)
        return false;
    await job.remove();
