...
export interface ListingVariations {
  ...
  keywords: string[]
  photoRanking: number[] // Indices of photos ranked by quality (0 = best)
 ...
 */
export async function generateListingVariations(options: GenerateListingOptions): Promise<ListingVariations> {
 ...
 */
export async function generateListingVariationsStream(
  options: GenerateListingOptions,
  onChunk: (chunk: string) => void
): Promise<ListingVariations> {
  const prompt = buildListingPrompt(options)
        ...
        res.on('data', (chunk) => {
          ...
          for (const line of lines) {
            if (line.startsWith('data:')) {
              try {
                const json = JSON.parse(line.slice(5)) as any
 ...
 */
function parseListingResponse(content: string, options: GenerateListingOptions): Omit<ListingVariations, 'estimatedTokens'> {
  try {
 ...
 */
export async function rankPhotosByQuality(imageUrls: string[]): Promise<number[]> {
