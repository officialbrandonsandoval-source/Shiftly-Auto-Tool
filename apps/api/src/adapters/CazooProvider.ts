...
import { ProviderAdapter, ProviderVehicle, ProviderCredentials, SyncResult } from './types.js'

export class CazooProvider implements ProviderAdapter {
  providerType = 'cazoo'
  private baseUrl = 'https://api.cazoo.co.uk'
  ...
  async fetchVehicles(credentials: ProviderCredentials): Promise<ProviderVehicle[]> {
    try {
      ...
      return response.items.map((item: any) => this.normalizeVehicle(item))
    } catch (err) {
  ...
  private normalizeCondition(condition: string | undefined): 'new' | 'used' | 'certified' {
    if (!condition) return 'used'
    const lower = condition.toLowerCase()
    if (lower.includes('new')) return 'new'
    if (lower.includes('certified') || lower.includes('cpo')) return 'certified'
    return 'used'
  }
}
