...
import { schedulePost, scheduleRecurringPosts, scheduleAutoRepost, getScheduledPosts } from './queue/scheduler.js'
...
app.post('/listings/:vehicleId/generate', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const { vehicleId } = req.params
...
app.get('/listings/:listingId', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const { listingId } = req.params

  try {
    const aiListing = getAIListing(listingId)
    if (!aiListing) return res.status(404).json({ error: 'AI listing not found' })
...
app.get('/vehicles/:vehicleId/listings', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const { vehicleId } = req.params
...
app.get('/auth/marketplace/facebook', (_req: Request, res: Response) => {
...
app.get('/auth/marketplace/facebook/callback', async (req: Request, res: Response) => {
...
app.post('/posts/:vehicleId/marketplace', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const { vehicleId } = req.params
  ...
  try {
    ...
    if (!fbConnection) return res.status(404).json({ error: 'Facebook connection not found' })
    ...
    if (!fbCredentials) {
      recordPostError(
        'temp-id',
        'Failed to decrypt Facebook credentials'
      )
      return res.status(500).json({ error: 'Failed to decrypt Facebook credentials' })
    }

    // Validate token is still valid
    const isValid = await validateAccessToken(fbCredentials as any)
    if (!isValid) {
    ...
    const result = await postToMarketplace(fbCredentials as any, {
      title: listing.title,
...
app.get('/posts/:vehicleId', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const { vehicleId } = req.params
...
app.delete('/posts/:postId', authMiddleware as any, (req: AuthRequest, res: Response) => {
...
app.get('/dealer/posts', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
...
app.post('/scheduler/post', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
...
app.post('/scheduler/recurring', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
  ...
  try {
    ...
    const result = await scheduleRecurringPosts(
      vehicleId,
      platform,
      listingId,
      connectionId,
...
app.post('/scheduler/auto-repost', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
...
app.get('/scheduler/jobs/:jobId', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const { jobId } = req.params

  try {
    // Check all queues for job
    let job = await postingQueue.getJob(jobId)
    if (!job) job = await retryQueue.getJob(jobId)
    if (!job) job = await analyticsQueue.getJob(jobId)
    if (!job) job = await repostQueue.getJob(jobId)
...
app.delete('/scheduler/jobs/:jobId', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const { jobId } = req.params
...
app.get('/scheduler/status', authMiddleware as any, async (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
...
app.get('/analytics', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
  const period = (req.query.period as 'daily' | 'weekly' | 'monthly' | 'all-time') || 'all-time'
...
app.get('/analytics/export', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
  const period = (req.query.period as 'daily' | 'weekly' | 'monthly' | 'all-time') || 'all-time'
...
app.post('/analytics/refresh', authMiddleware as any, (req: AuthRequest, res: Response) => {
  const dealerId = req.userId || 'default-dealer'
  const period = (req.body.period as 'daily' | 'weekly' | 'monthly' | 'all-time') || 'all-time'
