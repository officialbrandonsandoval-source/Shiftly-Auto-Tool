...
import { getPostsByDealer, Post } from './posts.js'
...
export interface DealerAnalytics {
  ...
  clickThroughRate: number // CTR = clicks / impressions
  conversionRate: number // CVR = conversions / clicks
...
const analyticsCache: Map<string, DealerAnalytics> = new Map()

/**
 * Generate analytics for a dealer
 */
export function generateDealerAnalytics(
  dealerId: string,
 ...
 */
export function getCachedAnalytics(dealerId: string, period: string): DealerAnalytics | null {
  const cacheKey = `${dealerId}-${period}`
 ...
 */
function generatePlatformBreakdown(posts: Post[]): PlatformMetrics[] {
 ...
 */
function generateVehiclePerformances(posts: Post[]): VehiclePerformance[] {
  const vehicleMap = new Map<string, Post[]>()
  ...
  vehicleMap.forEach((vehiclePosts, vehicleId) => {
    ...
    const firstPost = vehiclePosts.sort(
 ...
 */
function getDateRange(period: 'daily' | 'weekly' | 'monthly' | 'all-time'): {
  startDate: Date
  endDate: Date
} {
 ...
 */
export function exportAnalyticsCSV(analytics: DealerAnalytics): string {
  const rows: string[] = []
  ...
  analytics.platformBreakdown.forEach((p) => {
    rows.push(
      `${p.platform},${p.posts},${p.impressions},${p.clicks},${p.leads},${p.conversions},${p.ctr.toFixed(2)}%,${p.cvr.toFixed(2)}%`
    )
  })
  ...
  analytics.topPerformers.forEach((v) => {
    rows.push(
      `${v.year} ${v.make} ${v.model},${v.year},$${v.price},${v.impressions},${v.clicks},${v.conversions},${v.ctr.toFixed(2)}%,${v.daysListed}`
    )
  })
