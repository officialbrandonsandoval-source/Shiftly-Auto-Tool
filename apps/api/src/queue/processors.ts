 ...
 */
export async function processPostingJob(job: any): Promise<any> {
  ...
  try {
    ...
    if (platform === 'facebook_marketplace') {
      ...
      if (!credentials) throw new Error('Failed to decrypt Facebook credentials')

      const result = await postToMarketplace(credentials as any, {
 ...
 */
export async function processRetryJob(job: any): Promise<any> {
 ...
 */
export async function processAnalyticsJob(job: any): Promise<any> {
 ...
 */
export async function processRepostJob(job: any): Promise<any> {
...
async function getMarketplaceAnalytics(
  ...
  accessToken: string
): Promise<{ impressions: number; clicks: number; leads: number; conversions: number }> {
        ...
        try {
          ...
          if (json.error) {
            ...
            insights.forEach((insight: any) => {
              ...
              if (insight.name === 'clicks') clicks = insight.values[0]?.value || 0
            })
